...
function TerrainSurface({
  ...
  // A real side effect (notifying the parent), not a cached value -- useMemo
  // offers no guarantee it runs exactly once per change (React may discard
  // and recompute it, e.g. under Strict Mode's double-invoke), so this must
  // be useEffect, not useMemo.
