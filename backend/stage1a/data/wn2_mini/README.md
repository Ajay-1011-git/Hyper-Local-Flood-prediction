...
**The `.nc` file itself is intentionally not committed to git** (see
`backend/stage1a/../../.gitignore`'s `data/**` rule) — it's a manually
produced artifact, like the synthetic files in
`../gencast_precomputed/`. Copy your own export here before running the
backend, or ask a teammate for theirs. `WN2_MINI_FORECAST_PATH` in `.env`
